...
import Generated.Solver.structavail.Solve
...
structure StructAvail (P : Program) (avail : Node → Defs) : Prop where
  update : ∀ c c', Step P c c' → (avail c'.node).Subset (((localGen P c.node).union ((MTC.gatherS (allDefs P) (fun z => structSub P c.node z) (avail c.node)))))
  seed   : (avail P.entry).Subset (∅)
  within : ∀ n, (avail n).Subset (allDefs P)

theorem avail_iff_flow (P : Program) (h : Node → Defs) :
    StructAvail P h ↔ MTCSpec P (listNode P) (availT P) (∅) h := by
  constructor
  · intro hs
    refine ⟨?_, ?_, ?_⟩
...
theorem availSol_valid (P : Program) (wf : WellFormed P) : StructAvail P (availSol P) :=
  (avail_iff_flow P _).mpr (avail_correct P wf).1
theorem avail_greatest (P : Program) (wf : WellFormed P) :
    ∀ g, StructAvail P g → ∀ n, (g n).Subset (availSol P n) :=
