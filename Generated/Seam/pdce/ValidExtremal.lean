...
import Generated.Solver.pdce.Solve
...
structure Live (P : Program) (π : Node → Variables) : Prop where
  ...
  check   : ∀ n, (condVars P n).Subset (π n)
...
theorem π_iff_flow (P : Program) (h : Node → Variables) :
    Live P h ↔ MTCSpecB P (listVar P) (πT P) (condVars P) (liveSeed P) h := by
  constructor
  · intro hl
    refine ⟨?_, ?_, ?_, ?_⟩
...
/-! ## The uniform analysis interface: `PDCEResult` + `Valid`/`Extremal` + `PDCESolve`. -/

structure PDCEResult (P : Program) where
  π : Node → Variables
  η : Node → Assignments

def PDCEResult.Valid (P : Program) (S : PDCEResult P) : Prop :=
  (Live P S.π) ∧ (Sink P S.η)

def PDCEResult.Extremal (P : Program) (S : PDCEResult P) : Prop :=
  (∀ g, Live P g → ∀ n, (S.π n).Subset (g n)) ∧ (∀ g, Sink P g → ∀ n, (g n).Subset (S.η n))

def PDCESolve (P : Program) (wf : WellFormed P) : PDCEResult P where
  π := πSol P
  η := ηSol P

theorem PDCESolve_valid (P : Program) (wf : WellFormed P) : (PDCESolve P wf).Valid P :=
  ⟨πSol_valid P wf, ηSol_valid P wf⟩

theorem PDCESolve_extremal (P : Program) (wf : WellFormed P) : (PDCESolve P wf).Extremal P :=
  ⟨π_least P wf, η_greatest P wf⟩

#assert_clean_axioms PDCESolve_valid
#assert_clean_axioms PDCESolve_extremal
