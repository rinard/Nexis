...
def ηₘR (P : Program) : Config → Assignments → Config → Assignments → Prop :=
  fun c π c' π' => π'.Subset ((matPlace P (πₐSol P) (ηₐSol P) (ηₚSol P) (τₚSol P) (τᵤSol P) c.node c'.node).union (π.sdiff (notPass P c.node)))

theorem ηₘ_drives (P : Program) (wf : WellFormed P) : Drives P (ηₘR P) (ηₘSol P) :=
  fun c c' hstep => (ηₘSol_valid P wf).update c c' hstep

theorem ηₘ_preservation (P : Program) (a a' : Aug Assignments) (h : AugStep P (ηₘR P) a a') :
    Step P a.cfg a'.cfg := preservation h

theorem ηₘ_progress (P : Program) (wf : WellFormed P) (c c' : Config) (hs : Step P c c') :
    AugStep P (ηₘR P) ⟨c, ηₘSol P c.node⟩ ⟨c', ηₘSol P c'.node⟩ := progress (ηₘ_drives P wf) hs

theorem ηₘ_bisim (P : Program) (wf : WellFormed P) :
    (∀ c c', Step P c c' → AugStep P (ηₘR P) ⟨c, ηₘSol P c.node⟩ ⟨c', ηₘSol P c'.node⟩) ∧
    (∀ a a' : Aug Assignments, AugStep P (ηₘR P) a a' → Step P a.cfg a'.cfg) :=
  bisim (ηₘ_drives P wf)

#assert_clean_axioms ηₘ_drives
#assert_clean_axioms ηₘ_progress
