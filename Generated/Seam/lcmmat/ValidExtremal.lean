...
import Generated.Solver.lcmmat.Solve
...
structure Materialized (P : Program) (πₐ ηₐ ηₚ τₚ τᵤ : Node → Assignments) (ηₘ : Node → Assignments) : Prop where
  update : ∀ c c', Step P c c' → (ηₘ c'.node).Subset ((matPlace P πₐ ηₐ ηₚ τₚ τᵤ c.node c'.node).union ((ηₘ c.node).sdiff (notPass P c.node)))
  seed   : (ηₘ P.entry).Subset (entrySeed P)
  within : ∀ n, (ηₘ n).Subset (allExprs P)
...
theorem ηₘ_toSpec (P : Program) (πₐ ηₐ ηₚ τₚ τᵤ h : Node → Assignments) (hs : Materialized P πₐ ηₐ ηₚ τₚ τᵤ h) :
    MTCSpec P (listExpr P) (ηₘT P πₐ ηₐ ηₚ τₚ τᵤ) (entrySeed P) h :=
  ⟨fun c c' hstep x hx => hs.update c c' hstep x hx, fun x hx => hs.seed x hx,
   fun n x hx => Std.HashSet.mem_toList.mpr (hs.within n x hx)⟩
theorem ηₘ_ofSpec (P : Program) (πₐ ηₐ ηₚ τₚ τᵤ h : Node → Assignments) (hf : MTCSpec P (listExpr P) (ηₘT P πₐ ηₐ ηₚ τₚ τᵤ) (entrySeed P) h) : Materialized P πₐ ηₐ ηₚ τₚ τᵤ h :=
  ⟨fun c c' hstep x hx => hf.1 c c' hstep x hx, fun x hx => hf.2.1 x hx,
   fun n x hx => Std.HashSet.mem_toList.mp (hf.2.2 n x hx)⟩
theorem ηₘSol_valid (P : Program) (wf : WellFormed P) : Materialized P (πₐSol P) (ηₐSol P) (ηₚSol P) (τₚSol P) (τᵤSol P) (ηₘSol P) :=
  ηₘ_ofSpec P _ _ _ _ _ _ (ηₘ_correct P wf).1
theorem ηₘ_greatest (P : Program) (wf : WellFormed P) :
    ∀ g, Materialized P (πₐSol P) (ηₐSol P) (ηₚSol P) (τₚSol P) (τᵤSol P) g → ∀ n, (g n).Subset (ηₘSol P n) :=
  fun g hg n x hx => (ηₘ_correct P wf).2 g (ηₘ_toSpec P _ _ _ _ _ _ hg) n x hx

#assert_clean_axioms ηₘSol_valid
#assert_clean_axioms ηₘ_greatest

/-! ## The uniform analysis interface: `LcmMatResult` + `Valid`/`Extremal` + `LcmMatSolve`. -/

structure LcmMatResult (P : Program) where
  πₐ : Node → Assignments
  ηₐ : Node → Assignments
  ηₚ : Node → Assignments
  τₚ : Node → Assignments
...
def LcmMatResult.Valid (P : Program) (S : LcmMatResult P) : Prop :=
  (Anticipated P S.πₐ) ∧ (Available P S.ηₐ) ∧ (Postponable P S.πₐ S.ηₐ S.ηₚ) ∧ (Transfer Assignments.Subset P S.ηₚ S.τₚ) ∧ (Used P (latestNode P S.ηₚ S.τₚ) (latestEdge P S.πₐ S.ηₐ S.ηₚ) S.πᵤ) ∧ (Transfer Assignments.Sup P S.πᵤ S.τᵤ) ∧ (Materialized P S.πₐ S.ηₐ S.ηₚ S.τₚ S.τᵤ S.ηₘ)

def LcmMatResult.Extremal (P : Program) (S : LcmMatResult P) : Prop :=
  (∀ g, Anticipated P g → ∀ n, (g n).Subset (S.πₐ n)) ∧ (∀ g, Available P g → ∀ n, (g n).Subset (S.ηₐ n)) ∧ (∀ g, Postponable P S.πₐ S.ηₐ g → ∀ n, (g n).Subset (S.ηₚ n)) ∧ (∀ g, Transfer Assignments.Subset P S.ηₚ g → ∀ n, (g n).Subset (S.τₚ n)) ∧ (∀ g, Used P (latestNode P S.ηₚ S.τₚ) (latestEdge P S.πₐ S.ηₐ S.ηₚ) g → ∀ n, (S.πᵤ n).Subset (g n)) ∧ (∀ g, Transfer Assignments.Sup P S.πᵤ g → ∀ n, (S.τᵤ n).Subset (g n)) ∧ (∀ g, Materialized P S.πₐ S.ηₐ S.ηₚ S.τₚ S.τᵤ g → ∀ n, (g n).Subset (S.ηₘ n))

def LcmMatSolve (P : Program) (wf : WellFormed P) : LcmMatResult P where
  πₐ := πₐSol P
  ηₐ := ηₐSol P
  ηₚ := ηₚSol P
  τₚ := τₚSol P
...
theorem LcmMatSolve_valid (P : Program) (wf : WellFormed P) : (LcmMatSolve P wf).Valid P :=
  ⟨πₐSol_valid P wf, ηₐSol_valid P wf, ηₚSol_valid P wf, τₚSol_valid P wf, πᵤSol_valid P wf, τᵤSol_valid P wf, ηₘSol_valid P wf⟩

theorem LcmMatSolve_extremal (P : Program) (wf : WellFormed P) : (LcmMatSolve P wf).Extremal P :=
  ⟨πₐ_greatest P wf, ηₐ_greatest P wf, ηₚ_greatest P wf, τₚ_greatest P wf, πᵤ_least P wf, τᵤ_least P wf, ηₘ_greatest P wf⟩

#assert_clean_axioms LcmMatSolve_valid
#assert_clean_axioms LcmMatSolve_extremal
