...
import Generated.Solver.reaching.Solve
...
structure Reaching (P : Program) (reach : Node → Defs) : Prop where
  update : ∀ c c', Step P c c' → ((genDefs P c.node).union ((reach c.node).inter (transpDefs P c.node))).Subset (reach c'.node)
  seed   : Defs.Subset (∅) (reach P.entry)
  within : ∀ n, (reach n).Subset (allDefs P)

theorem reach_iff_flow (P : Program) (h : Node → Defs) :
    Reaching P h ↔ MTCSpecMF P (listNode P) (reachT P) (∅) h := by
  constructor
  · intro hs
    refine ⟨?_, ?_, ?_⟩
...
theorem reachSol_valid (P : Program) (wf : WellFormed P) : Reaching P (reachSol P) :=
  (reach_iff_flow P _).mpr (reach_correct P wf).1
theorem reach_least (P : Program) (wf : WellFormed P) :
    ∀ g, Reaching P g → ∀ n, (reachSol P n).Subset (g n) :=
