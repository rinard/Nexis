...
structure PDCEFaultBitvec (P : Program) where
  π : Node → BVVar P
  η : Node → BVAsgn P

def solve (P : Program) : PDCEFaultBitvec P :=
  let arr_π := solveMTCB P (listVar P) (πT P) (liveFloorF P) (liveSeed P)
  let f_π : Node → BVVar P := fun n => if n < P.size then gA (arr_π) n else encode (listVar P) ((liveFloorF P) n)
...
theorem π_correct (P : Program) (wf : WellFormed P) :
    MTCSpecB P (listVar P) (πT P) (liveFloorF P) (liveSeed P) (πSol P) ∧
    ∀ h, MTCSpecB P (listVar P) (πT P) (liveFloorF P) (liveSeed P) h → ∀ n, ∀ x ∈ πSol P n, x ∈ h n :=
  resMTCB_correct (lo := liveFloorF P) (sd := liveSeed P) wf (listVar_nodup P) (πWf P)
    (fun n x hx => Std.HashSet.mem_toList.mpr (liveFloorF_sub P n x hx))
