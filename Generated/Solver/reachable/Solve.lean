...
def reachT (P : Program) : MTC Node :=
  .union (.const (fun (n, _) => genNode P n)) (.inter .var (.const (fun (n, _) => transpAllNodes P n)))
...
structure ReachableBitvec (P : Program) where
  reach : Node → BVNode P

def solve (P : Program) : ReachableBitvec P :=
...
theorem reachWf (P : Program) : MTC.Wf (listNode P) (reachT P) :=
  ⟨fun (n, _) x hx => Std.HashSet.mem_toList.mpr (genNode_sub P n x hx), ⟨by trivial, fun (n, _) x hx => Std.HashSet.mem_toList.mpr (transpAllNodes_sub P n x hx)⟩⟩
