...
* The **LCM** step is the KRS lazy-code-motion / PRE hoisting transform (`Analyses.LCM.transform`, the two-valued
  ...
  run over the bundle `Analyses.LcmMat.lcmMatSolved` (`--lcm-analysis` selects it; see below).
...
**bundle** inside `Solver/{lcmmat,pdce}/Solve.lean`, and `lcmMatSolved`/`pdceSolved` (the adapters, in
`Seam/{lcmmat,pdce}/Adapter.lean`) project each ghost + carry its validity/extremality witness.
The witnesses are `sorry`-free under `WellFormed`, discharged here by `normalize_lower_wellNormalized` +
`transform_wellFormed`. Each transform is parameterized over an arbitrary **valid** bundle and is
verified correct for it (LCM: `transform_preserves_halt`); extremality is what `Optimality.lean`
quantifies over, not what correctness needs.
-/
...
def optStages (ana : Analyses.LcmMat.LcmAnalysis) (gate : Analyses.LCM.GateMode)
    (mode : Analyses.LCM.LcmMode)
  ...
  (TextToAst.parse src).map fun s =>
    ...
    -- LCM (PRE / lazy code motion) hoist. `lcmMatSolved`/`pdceSolved` are the bundle-backed adapters
    -- (`Seam/{lcmmat,pdce}/Adapter.lean`): the memoizing `solve` bundle computed once, each ghost
    -- projected as `decF(solve).x` with its validity witness from `Solve.lean`. Each verified
    -- `transform` is parameterized over an arbitrary valid bundle.
    ...
    let bL := (ana.bundle P₀ wf₀).withGate gate
...
/-- The full source-to-assembler pipeline; `none` on a parse error. `clean` (on by default) runs the
    ...
    by `pipeline_to_asm`. -/
def compile (ana : Analyses.LcmMat.LcmAnalysis) (gate : Analyses.LCM.GateMode)
    (mode : Analyses.LCM.LcmMode)
    (pmode : Analyses.PDCE.PdceMode) (clean : Bool)
    (src : String) : Option String :=
  (optStages ana gate mode pmode src).map (fun (_, _, final) =>
...
def showOpt (ana : Analyses.LcmMat.LcmAnalysis) (gate : Analyses.LCM.GateMode)
    (mode : Analyses.LCM.LcmMode)
    (pmode : Analyses.PDCE.PdceMode) (src : String) : IO UInt32 :=
  match optStages ana gate mode pmode src with
...
def emitStage (ana : Analyses.LcmMat.LcmAnalysis) (gate : Analyses.LCM.GateMode)
    (mode : Analyses.LCM.LcmMode) (pmode : Analyses.PDCE.PdceMode) (lowered : Bool)
...
def main (args : List String) : IO UInt32 := do
  ...
  -- `analyses/lcm/Lcm.gsl`, six ghosts) or `mat` / `materialized` (`analyses/lcmmat/LcmMat.gsl`, the
  -- same six plus the `Materialized` availability ghost `ηₘ`).
  ...
  let ana ← match anaArg with
    ...
    | a :: _ =>
      ...
      | none   => do
          ...
          IO.Process.exit 1
  -- `--lcm-gate=<name>`: which ghost the transform's **replace gate** reads. This is the axis the
  -- development is about, and both settings are shipped and proved:
  --   `demand`       — the classical KRS gate `πᵤK ∪ insertBefore`. `πᵤ` is a *least* fixpoint, so
  --                    correctness needs `Extremal S` (`Compile.main_compile_correct_demand`).
  --   `materialized` — `ηₘK ∪ insertBefore`, reading the seventh ghost. Its clause is an *upper* bound,
  --                    so correctness needs only **validity** (`Compile.main_compile_correct`).
  -- The default follows the analysis: `classic` ships `.demand` (it has no `ηₘ` to read), `mat` ships
  -- `.materialized`. On an extremal bundle the two gates emit the same code (`demand_materialized`).
  let gateArg := args.filter (fun a => a.startsWith "--lcm-gate=")
  let gate ← match gateArg with
    | []     => pure (ana.bundleGate)
    | a :: _ =>
      let spelling := (a.drop "--lcm-gate=".length).toString
      match Analyses.LCM.GateMode.ofString? spelling with
      | some g => pure g
      | none   => do
          IO.eprintln s!"prophecyc: unknown --lcm-gate '{spelling}' \
                        (expected: demand | classic | mat | materialized)"
          IO.Process.exit 1
  ...
  if args.contains "--emit-lowered" then
    emitStage ana gate mode pmode true src
  else if args.contains "--emit-opt" then
    emitStage ana gate mode pmode false src
    ...
    showOpt ana gate mode pmode src
  else
    match compile ana gate mode pmode clean src with
    | some asm => IO.print asm; pure 0
    | none     => IO.eprintln "prophecyc: parse error"; pure 1
